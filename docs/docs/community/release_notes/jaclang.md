...
- **Fix: Ternary (If-Else) Expression Type Inference**: The type checker now infers the result type of ternary expressions (`x if cond else y`). When both branches have the same type, it returns that type directly; when they differ, it produces a union type. Previously ternary expressions were untyped, causing false positive type errors on subsequent assignments.
