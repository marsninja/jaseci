...
- **Refactor: CWD-Local Bytecode Cache**: Moved the bytecode and IR disk cache from a global user directory (`~/.cache/jac/bytecode/`) to a CWD-relative `.jac/cache/bytecode/` directory. Since precompiled bytecode is now bundled into PyPI packages, the disk cache is only used during editable (dev) installs. A local cache gives developers natural isolation between branches and worktrees. Removed the platform-specific `get_global_cache_dir()` and `_get_bootstrap_cache_dir()` helpers.
