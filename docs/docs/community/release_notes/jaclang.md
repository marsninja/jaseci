...
- **Type Checker: Reduced False Positives**: Fixed false "Cannot assign type" errors by addressing two root causes: (1) cross-module class identity -- same class loaded from different paths now correctly recognized via normalized module path comparison, and (2) protocol conformance checks now walk the full MRO chain so inherited methods are found.
