...
- **Native Codegen: Expanded Primitive Coverage**: Added 25 new LLVM IR emitter implementations across 7 emitters: IntEmitter (`conjugate`, `bit_length`, `bit_count`), FloatEmitter (`conjugate`, `is_integer`, `op_floordiv`), StrEmitter (`title`, `rfind`, `ljust`, `rjust`, `zfill`, `index`, `rindex`, `center`, `isidentifier`, `istitle`), SetEmitter (`clear`, `discard`, `copy`), ListEmitter (`extend`, `insert`, `remove`), DictEmitter (`clear`, `copy`), and BuiltinEmitter (`bool`). Added dict method dispatch through the emitter layer. `str.index`/`rindex` raise `ValueError` on missing substring, and `str.center` matches CPython's odd-padding formula. Fixed string comparison operators, float floor division, and `rfind` infinite loop on empty substring. 121/299 implemented (40%), 114/299 tested (38%).
