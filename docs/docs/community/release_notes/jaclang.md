...
- **Fix: PyastGenPass IndexError with Decl+Impl Separation**: Fixed `exit_func_signature` crashing with `IndexError: list index out of range` when compiling modules that use declaration/implementation separation (`.impl.jac` annex files). On re-traversal of annex `ImplDef` nodes, child `ParamVar` nodes had empty `gen.py_ast`; added a `_param_arg()` helper that synthesizes `ast3.arg` directly from the node's name and type annotation.
- **Fix: Native Codegen Phase Ordering for Struct-Typed Globals**: Fixed `glob game: Game = Game()` failing with "cannot store Game*to i64*" by pre-registering opaque struct types before processing global variable declarations. Added `_pre_register_struct_types()` pass that creates identified struct types early so globals with struct-typed annotations resolve correctly.
- **Fix: Native Codegen Signature-less Methods**: Fixed segfault when compiling methods without parentheses (e.g., `def postinit { ... }`). These have no `FuncSignature` node; relaxed the signature filter in archetype registration and updated `_forward_declare_method`/`_codegen_method` to handle `sig is None`.
- **Fix: Native Codegen Global Type Inference from Constructors**: Untyped global variables initialized with constructor calls (e.g., `glob game = Game()`) now infer the correct LLVM pointer type from the struct type registry instead of defaulting to `i64`.
