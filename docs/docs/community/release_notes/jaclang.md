...
- **Type Checker: Reduced False Positives**: Fixed false "Cannot assign type" errors by addressing three root causes: (1) cross-module class identity -- same class loaded from different paths now correctly recognized via normalized module path comparison, (2) protocol conformance checks now walk the full MRO chain so inherited methods are found, and (3) annotated assignments (`x: T | None = None`) now use the declared type annotation instead of the resolved attribute type.
