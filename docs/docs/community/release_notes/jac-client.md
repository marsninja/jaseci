...
- **Desktop App Dynamic Port Discovery**: Desktop apps (Tauri) now dynamically discover the sidecar backend port at runtime instead of hardcoding port 8000. The sidecar starts with `--port 0` so the OS assigns a free port, prints `JAC_SIDECAR_PORT=<port>` to stdout, and the Tauri host reads it before injecting `globalThis.__JAC_API_BASE_URL__` into the webview via `webview.eval()`. This fixes desktop apps failing to connect to their backend when port 8000 was already in use. When a `base_url` is configured in `jac.toml`, the sidecar is skipped entirely and the configured URL is used directly.
