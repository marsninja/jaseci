...
- **Fixed sidecar broken pipe crash**: The sidecar now uses `jaclang.cli.console` for all status and error output after the runtime is imported, and `sys.stderr.write()` for early errors before jaclang is available. The only stdout write is the `JAC_SIDECAR_PORT=` marker line (via `sys.stdout.write()`). Previously, after the Tauri host read the port marker from stdout and closed the pipe, subsequent stdout writes from the sidecar raised `BrokenPipeError` and crashed the process before the API server could start.
