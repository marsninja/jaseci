...
- **Desktop App Dynamic Port Discovery**: Desktop apps (Tauri) now dynamically discover the sidecar backend port at runtime instead of hardcoding port 8000. The sidecar starts with `--port 0` so the OS assigns a free port, prints `JAC_SIDECAR_PORT=<port>` to stdout, and the Tauri host reads it before injecting `globalThis.__JAC_API_BASE_URL__` into the webview via `WebviewWindowBuilder::initialization_script()`. This guarantees the API base URL is set before any page JavaScript executes, fixing the race condition where `webview.eval()` ran too late and API calls used same-origin (the Tauri dev server) instead of the backend. When a `base_url` is configured in `jac.toml`, the sidecar is skipped entirely and the configured URL is used directly.
- **Unified API Base URL Resolution**: Extracted a single `_resolve_api_base_url()` method in `ViteBundler` that implements a consistent priority chain: `jac.toml` `base_url` > direct override > `JAC_CLIENT_API_BASE_URL` env var > empty string (same-origin). Both `create_vite_config()` and `create_dev_vite_config()` now delegate to this method, eliminating duplicated resolution logic. The `DesktopTarget.dev()` and `DesktopTarget.start()` methods now accept an `api_port` parameter, passed through from the CLI `--port` flag.
- **Fixed environment variable leak in desktop build/start**: The `JAC_CLIENT_API_BASE_URL` environment variable set during `DesktopTarget.build()` and `DesktopTarget.start()` is now cleaned up in a `finally` block, preventing it from leaking into subsequent operations if the web bundle build throws an exception.
- **Fixed sidecar broken pipe crash**: The sidecar now writes all status and error messages to stderr instead of stdout. Previously, after the Tauri host read the port marker from stdout and closed the pipe, subsequent stdout writes from the sidecar raised `BrokenPipeError` and crashed the process before the API server could start. The `jaclang.cli.console` import was also removed because its `print()` method silently ignores the `file=sys.stderr` parameter in the base implementation.
- **Desktop backend auto-start**: `DesktopTarget.start()` and `DesktopTarget.dev()` now automatically launch the Jac backend API server as a subprocess before opening the Tauri window. The subprocess passes `--no_client` to skip redundant client bundling (the desktop target already builds the web bundle separately). The backend port is resolved from `jac.toml` `base_url` or falls back to the `--port` flag. Both methods cleanly terminate the backend on shutdown.
- **Skip client bundling with `--no_client`**: The `server.start()` method now accepts a `no_client` parameter. When `True`, the eager `ensure_bundle()` call is skipped, preventing duplicate client builds when the caller has already built the bundle (e.g., desktop target). The `jac start --no_client` CLI flag is now forwarded through to the server.
