...
- **Simplified Client Project Structure**: The `jac create --cl` command now creates a flatter project structure with `main.jac` at the project root instead of `src/app.jac`, and `components/` at the root instead of `src/components/`. This reduces unnecessary nesting and aligns with modern frontend conventions while maintaining full backward compatibility with existing `src/` based projects.
