...
- **Node.js Detection and Auto-Installation**: Added intelligent Node.js detection when creating client projects with `jac create --cl`. If Node.js or npm is not found, the CLI now provides platform-specific installation instructions (macOS, Linux, Windows) including package managers (brew, apt, dnf, winget, choco) and nvm. A new `--setup-node` flag enables automatic Node.js installation via nvm on macOS and Linux: `jac create --cl --setup-node myapp`. This streamlines the onboarding experience for new developers.
