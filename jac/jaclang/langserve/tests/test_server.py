...
class TestJacLangServer(TestCase):
    ...
    def test_impl_stay_connected(self) -> None:
        ...
        self.assertIn(
            ...
            self.lsp.get_hover_info(circle_file, pos).contents.value,
        )
        ...
        self.assertIn(
            ...
            self.lsp.get_hover_info(circle_impl_file, pos).contents.value.replace(
                "'", ""
            ),
        )
    ...
    def test_outline_symbols(self) -> None:
        ...
        circle_file = uris.from_fs_path(self.fixture_abs_path("circle_pure.jac"))
        self.lsp.type_check_file(circle_file)
        self.assertEqual(8, len(self.lsp.get_outline(circle_file)))
    ...
    def test_go_to_definition(self) -> None:
        ...
        self.assertIn(
            ...
            str(self.lsp.get_definition(circle_file, lspt.Position(9, 16))),
        )
        self.assertIn(
            "fixtures/circle_pure.jac:13:11-13:16",
            str(self.lsp.get_definition(circle_file, lspt.Position(20, 16))),
        )
        ...
        self.assertIn(
            ...
            str(self.lsp.get_definition(goto_defs_file, lspt.Position(4, 14))),
        )
        # Test if the here keywrod goes to the node definition
        self.assertIn(
            "fixtures/goto_def_tests.jac:0:5-0:13",
            str(self.lsp.get_definition(goto_defs_file, lspt.Position(10, 14))),
        )
        # Test the SomeNode node inside the visit statement goes to its definition
        self.assertIn(
            "fixtures/goto_def_tests.jac:0:5-0:13",
            str(self.lsp.get_definition(goto_defs_file, lspt.Position(11, 21))),
        )
    ...
    def test_go_to_definition_method_manual_impl(self) -> None:
        ...
        self.assertIn(
            ...
            str(self.lsp.get_definition(decldef_file, lspt.Position(2, 20))),
        )
    ...
    def test_completion(self) -> None:
        ...
        # Use the existing event loop owned by the server
        for case in test_cases:
            results = self.lsp.loop.run_until_complete(
